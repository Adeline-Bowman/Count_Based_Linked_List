...
void LinkedListCountBased::append_command(string command) {
...
void LinkedListCountBased::reorganize() {
    ...
    while (current) {
        node* nextNode = (current) ? current->next : nullptr; // Ensure nextNode is initialized safely

        // Find the correct position to move current node forward.
        node* temp = (current->prev) ? current->prev : nullptr; //Ensure temp is initialized safely
